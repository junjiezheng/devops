pipeline{
	...
	stages{
		...
		stage('通过Publish Over SSH通知目标服务器'){
			steps{
				sshPublisher paramPublish: [parameterName: 'BUILD_DEV'], publishers: [sshPublisherDesc(configName: 'devops_dev', sshLabel: [label: 'dev'], transfers: [sshTransfer(cleanRemote: false, excludes: '', execCommand: '''cd /usr/local/test/docker
				mv ../target/*.jar ./
				docker-compose down
